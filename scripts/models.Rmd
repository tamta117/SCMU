...
```{r m5}
m5 <- glmer(SCMU ~ time_since + time_int + tod + jday + jday^2 + #distance + 
              (1 | cam_id), 
...
```{r m6}
cora <- acoustic %>% filter(CORA == 1) # there are only 3 CORA acoustic detections!

m6 <- glmer(CORA ~ time_since + time_int + tod + jday + jday^2 + #distance +
               (1 | cam_id),
             data = acoustic, family = binomial)
 summary(m6)
```