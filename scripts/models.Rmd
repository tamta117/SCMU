...
camera_hourly$l.photocount<-log(camera_hourly$photo_count)
m1 <- glmer(SCMU_hourly_det ~ image_type + tod + jday + jday^2 + 
              (1 | cam_id), 
            data = camera_hourly, family = binomial)
m1.5 <- glmer(SCMU_hourly_det ~ image_type + image_type : l.photocount + tod + jday + jday^2 + 
              (1 | cam_id), 
            data = camera_hourly, family = binomial)
summary(m1)
...
```{r mod2}
m2 <- glmer(CORA_hourly_det ~ image_type + image_type:photo_count + tod + jday + jday^2 + (1 | cam_id), 
            data = camera_hourly, family = binomial)
summary(m2)
...
m5 <- glmer(SCMU ~ time_int + tod + jday + jday^2 + #distance + 
              (1 | cam_id), 
...
m6 <- glmer(CORA ~ time_int + tod + jday + jday^2 + #distance +
               (1 | cam_id),
             data = acoustic, family = binomial)
 summary(m6)
```